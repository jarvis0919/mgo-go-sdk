...
type ISubscribeAPI interface {
	SubscribeEvent(ctx context.Context, req request.MgoSubscribeEventsRequest, msgCh chan respone.MgoEventResponse) error
	SubscribeTransaction(ctx context.Context, req request.MgoSubscribeTransactionsRequest, msgCh chan respone.MgoEffects) error
}
...
func (s *mgoSubscribeImpl) SubscribeEvent(ctx context.Context, req request.MgoSubscribeEventsRequest, msgCh chan respone.MgoEventResponse) error {
	...
	go func() {
		for {
			...
			case messageData := <-rsp:
				var result respone.MgoEventResponse
...
func (s *mgoSubscribeImpl) SubscribeTransaction(ctx context.Context, req request.MgoSubscribeTransactionsRequest, msgCh chan respone.MgoEffects) error {
	rsp := make(chan []byte, 10)
	err := s.conn.Call(ctx, wsconn.CallOp{
